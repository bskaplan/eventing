...
type OnDiskChannelLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.OnDiskChannel, err error)
...
// List lists all OnDiskChannels in the indexer.
func (s *onDiskChannelLister) List(selector labels.Selector) (ret []*v1alpha1.OnDiskChannel, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.OnDiskChannel))
	})
	return ret, err
}
...
type OnDiskChannelNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.OnDiskChannel, error)
...
// List lists all OnDiskChannels in the indexer for a given namespace.
func (s onDiskChannelNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.OnDiskChannel, err error) {
...
// Get retrieves the OnDiskChannel from the indexer for a given namespace and name.
func (s onDiskChannelNamespaceLister) Get(name string) (*v1alpha1.OnDiskChannel, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("ondiskchannel"), name)
	}
	return obj.(*v1alpha1.OnDiskChannel), nil
}
