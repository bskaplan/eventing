...
// controller reconciling v1beta1.OnDiskChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1beta1.OnDiskChannel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1beta1.OnDiskChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1beta1.OnDiskChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1beta1.OnDiskChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1beta1.OnDiskChannel) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1beta1.OnDiskChannel resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister messagingv1beta1.OnDiskChannelLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister messagingv1beta1.OnDiskChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(existing *v1beta1.OnDiskChannel, desired *v1beta1.OnDiskChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.MessagingV1beta1().OnDiskChannels(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.MessagingV1beta1().OnDiskChannels(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1beta1.OnDiskChannel) (*v1beta1.OnDiskChannel, error) {
	...
	resource, err = r.Client.MessagingV1beta1().OnDiskChannels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1beta1.OnDiskChannel) (*v1beta1.OnDiskChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1beta1.OnDiskChannel, reconcileEvent reconciler.Event) (*v1beta1.OnDiskChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
