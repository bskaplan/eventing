...
func (odc *OnDiskChannel) Validate(ctx context.Context) *apis.FieldError {
	errs := odc.Spec.Validate(ctx).ViaField("spec")

	// Validate annotations
	if odc.Annotations != nil {
		if scope, ok := odc.Annotations[eventing.ScopeAnnotationKey]; ok {
...
func (odcs *OnDiskChannelSpec) Validate(ctx context.Context) *apis.FieldError {
	var errs *apis.FieldError

	if odcs.Subscribable != nil {
		for i, subscriber := range odcs.Subscribable.Subscribers {
