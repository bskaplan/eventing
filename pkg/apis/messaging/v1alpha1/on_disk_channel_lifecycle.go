...
func (odcs *OnDiskChannelStatus) SetAddress(url *apis.URL) {
	if odcs.Address == nil {
		odcs.Address = &v1alpha1.Addressable{}
	}
	if url != nil {
		odcs.Address.Hostname = url.Host
		odcs.Address.URL = url
		odcCondSet.Manage(odcs).MarkTrue(OnDiskChannelConditionAddressable)
	} else {
		odcs.Address.Hostname = ""
		odcs.Address.URL = nil
...
func (odcs *OnDiskChannelStatus) PropagateDispatcherStatus(ds *appsv1.StatefulSetStatus) {
	for _, cond := range ds.Conditions {
		// TODO: Once K8s defines StatefulSetConditionTypes explicitly, switch to that.
