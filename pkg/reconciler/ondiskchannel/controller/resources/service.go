...
func NewK8sService(odc *v1alpha1.OnDiskChannel, opts ...K8sServiceOption) (*corev1.Service, error) {
	// Add annotations
	svc := &corev1.Service{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      CreateChannelServiceName(odc.ObjectMeta.Name),
			Namespace: odc.Namespace,
			Labels: map[string]string{
				MessagingRoleLabel: MessagingRole,
			},
			OwnerReferences: []metav1.OwnerReference{
				*kmeta.NewControllerRef(odc),
			},
		},
